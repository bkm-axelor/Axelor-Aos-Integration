...
public class SaleOrderReport extends BatchStrategy {
  ...
  protected void process() {
    ...
    Batch recentRanBatch =
        Beans.get(BatchRepository.class)
            .all()
            .filter("self.saleBatch != null")
            .order("-updatedOn")
            .fetch(2, 1)
            .get(0);
    ...
    if (partner != null) {
      saleOrders =
          Beans.get(SaleOrderRepository.class)
              ...
              .bind("updatedOn", recentRanBatch.getEndDate())
              .fetch();
    ...
    for (SaleOrder saleOrder : saleOrders) {
      try {
        Template template =
            Beans.get(TemplateRepository.class).findByName("SaleOrderReportAndSendMail2");
        Message message =
            Beans.get(TemplateMessageService.class).generateMessage(saleOrder, template);
